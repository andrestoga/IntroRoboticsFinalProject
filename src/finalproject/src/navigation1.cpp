...
void MoveSlowly(float t);
void Wait(float t);
int moveGoal(geometry_msgs::Point goal_pose, double distance_tolerance);
	...
	while (ros::ok())
  	{
  		...
  		else if (0 == global_state)
  		{
			...
			// MoveSlowly(1.0);
			// Wait(3.0);
			rotate( degrees2radians(ROTATE_SPEED_DEG_PS), degrees2radians(rand() % RANDOM_ANGLES_RANGE + MIN_ROT_ANGLE), 1);
	...
	finalproject::Navigator::Response &res)
{
	...
	if ( 1 == global_state  && !wrongLocalization)
	{
		...
		res.info = moveGoal(req.point, .01);
...
int moveGoal(geometry_msgs::Point goal_pose, double distance_tolerance)
{
	...
	do{
		//linear velocity 
		vel_msg.linear.x = FORWARD_SPEED_M_PS_LOC *getDistance(position.position.x, position.position.y, goal_pose.x, goal_pose.y);
		vel_msg.linear.y = 0;
		vel_msg.linear.z = 0;
		//angular velocity
		vel_msg.angular.x = 0;
		vel_msg.angular.y = 0;
		vel_msg.angular.z = .5*(atan2(goal_pose.y - position.position.y, goal_pose.x - position.position.x)-robotAngle);
	...
	}while(getDistance(position.position.x, position.position.y, goal_pose.x, goal_pose.y) > distance_tolerance);
	ROS_INFO("End move goal");
	vel_msg.linear.x = 0;
	vel_msg.angular.z = 0;
	vel_Pub.publish(vel_msg);
	...
	if ( 1 == isObstacle )
	{
		ROS_INFO("Obstable found during the path!!!");
		return 1;
	}
	else
	{
